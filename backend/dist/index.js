...
import { initDatabase, getDesigns, getDesignById, saveDesign, deleteDesign, createUser, getUserByUsername, getUsers, updateUserRole, getComments, addComment, getLikesCount, hasUserLiked, toggleLike, getVersions, getVersionById, saveVersion } from './database.js';
dotenv.config();
...
async function startServer() {
    try {
        ...
        let esp32Ws = null; // the one ESP32 connection
        ...
        function broadcastStatus(connected, reconnecting = false) {
        ...
        wss.on('connection', (ws, req) => {
            ...
            ws.on('message', (raw) => {
                ...
                try {
                    ...
                    console.log('[ERR] JSON parse error: ' + e.message);
                    return;
                }
                ...
                if (data.client === 'esp32') {
                    ...
                    if (data.rssi !== undefined)
                        lastRssi = data.rssi;
                    console.log(`[ESP32] Initial state → LED=${lastLedState} brightness=${lastBrightness} rssi=${lastRssi}`);
                    // Restore last LED state from backend on reconnect/connect (Requirement 7)
                    esp32Ws.send(JSON.stringify({ type: 'set_led', state: lastLedState }));
                    esp32Ws.send(JSON.stringify({ device: 'esp32', pin: 2, state: lastLedState }));
                ...
                if (data.device === 'esp32') {
                    console.log(`[FORWARD] Sending to ESP32: pin=${data.pin} state=${data.state}`);
